...
CConfigWnd::CConfigWnd() : m_pColorComboTitle(NULL), m_pColorComboCount(NULL), m_pColorComboDay(NULL),  m_pMainUI(NULL)
{
...
{
	return (gPlugInfo.Path + _T("Res\\PlugCfgDeskCount.xml")).c_str();
}
...
void CConfigWnd::OnOK()
{
	...
	CTime EndDateTime = CTime(endDate.wYear, endDate.wMonth, endDate.wDay, 0, 0, 0);
...
void CConfigWnd::InitWindow()
{
	...
	m_pColorComboDay->SetSelColor(RGB(255, 255, 255));
...
void CConfigWnd::SaveCfg()
{
	...
	gUnify->DB()->PlugDB()->SetInt(sSection.c_str(), _T("m_pColorComboDay"), m_pColorComboDay->GetSelColor());
	...
	SetParamByCheckCtrl(L"chkShowDayLabel");
...
void CConfigWnd::LoadCfg()
{
	...
	m_pColorComboDay->SetSelColor(gUnify->DB()->PlugDB()->GetInt(sSection.c_str(), _T("m_pColorComboDay"), RGB(255, 255, 255)));
	...
	SetCheckCtrlByParam(L"chkShowDayLabel");
...
void CConfigWnd::TryGetControls()
{
	...
	m_pColorComboDay = static_cast<CColorComboUI*>(m_PaintManager->FindControl(_T("cmbColorDay")));
